...
function Caret(owner) {
  ...
  this.relative = relative_coords(this);
};
...
Caret.prototype.relative = function() {
  var lines = this.owner.lines();
...
// Utility function to return an object literal representing the relative
// coordinates for a given caret object.
var relative_coords = function(caret) {
  return {
    get x() {
      var lines = caret.owner.lines();
      var seg = lines[caret.absolute.y].slice(caret.owner.offset.x,
                                              caret.absolute.x);
      return caret.owner.strWidth(seg);
    },
    get y() {
      return caret.owner.offset.y;
    }
  };
};
