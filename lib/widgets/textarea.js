...
Textarea.prototype._listener = function(ch, key) {
  ...
  case 'backspace':
    ...
    this.value = editStringDelete(value, len, caret);
    pressed.key = 'backspace';
    pressed.length = len;
    break;
  default:
    if (ch && !/^[\x00-\x08\x0b-\x0c\x0e-\x1f\x7f]$/.test(ch)) {
      this.value = editStringInsert(value, ch, caret);
...
// Utility function to convert a width of characters (width) on a line (line) to
// the length of those characters.
var convertWidthToLength = function(widget, line, width) {
  if (widget.strWidth(line) <= width) return line.length;

  var len = 0;
  for (var i = 0; i < width; i++) {
    len += calculateNextGlyphLength(widget.screen.fullUnicode, line, len);
  }
  return len;
};
...
var moveCoordsDown = function(widget, lines, original) {
  ...
  var x = convertWidthToLength(widget, lines[y], width);
  return coords(x, y);
};
