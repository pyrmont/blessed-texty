...
Textarea.prototype._listener = function(ch, key) {
  ...
  case 'left':
    if (!value.length) break;
    pressed.key = 'left';
    pressed.width = this._getPrevGlyphWidth(fullUnicode, value, caret);
    break;
  case 'right':
    if (!value.length) break;
    pressed.key = 'right';
    pressed.width = this._getNextGlyphWidth(fullUnicode, value, caret);
    break;
  case 'backspace':
    if (!value.length) break;
    ...
    var width = this._getPrevGlyphWidth(fullUnicode, value, caret);
    this.value = this._deleteString(value, width, caret);
...
Textarea.prototype._getCoordsAfterDown = function(lines, coords) {
  ...
  if (coords.y + 1 == lines.length) {
    ...
    // This doesn't handle double-width characters at all.
    new_coords.x = (coords.x > lines[new_coords.y].length) ? lines[new_coords.y].length : coords.x;
  }
  return new_coords;
}
