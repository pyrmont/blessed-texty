...
Textarea.prototype._updateCursor = function(get) {
  ...
  var offset = this._convertToOffset(this._clines, this.cursor);
...
Textarea.prototype._listener = function(ch, key) {
  ...
  case 'left':
    if (!value.length) break;
    pressed.key = 'left';
    pressed.length = this._getPrevGlyphLength(fullUnicode, value, caret);
    break;
  case 'right':
    if (!value.length) break;
    pressed.key = 'right';
    pressed.length = this._getNextGlyphLength(fullUnicode, value, caret);
    break;
  case 'backspace':
    if (!value.length) break;
    ...
    var len = this._getPrevGlyphLength(fullUnicode, value, caret);
...
Textarea.prototype._getCoordForWidth = function(line, width) {
  ...
  for (var i = 0; i < width; i++) {
    x += this._getNextGlyphLength(this.screen.fullUnicode, line, x);
  }
  return x;
}
...
Textarea.prototype._getCoordsAfterDown = function(lines, coords) {
  var new_coords = { x: null, y: null };
  if (coords.y + 1 == lines.length) {
    new_coords.y = coords.y;
    new_coords.x = coords.x;
  } else {
    new_coords.y = coords.y + 1;
    var width = this.strWidth(lines[coords.y].slice(0, coords.x));
    new_coords.x = this._getCoordForWidth(lines[new_coords.y], width);
  }
  return new_coords;
}
...
Textarea.prototype._getCoordsAfterUp = function(lines, coords) {
  var new_coords = { x: null, y: null };
  if (coords.y == 0) {
    new_coords.y = coords.y;
    new_coords.x = coords.x;
...
Textarea.prototype._getCoordsForPosition = function(lines, text, pos) {
  return this._convertPositionToCoords(lines, text, pos);
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    if (agg_len == pos) {
      ...
      coords.y = i;
      coords.x = pos - prev_agg_len;
      break;
    }
  }

  if (coords.y == null || coords.x == null) throw 'Coordinates not set';
...
Textarea.prototype._convertPositionToCoords = function(lines, text, pos) {
  ...
  for (i = 0; i < lines.length; i++) {
    ...
    if (chars == pos) {
      return (i + 1 < lines.length) ? coords(0, i + 1) : coords(j, i);
    } else if (chars > pos) {
      return coords(j, i);
    }
  }
...
Textarea.prototype._getPositionForCoords = function(lines, text, coords) {
  var agg_len = 0;
  ...
  if (text.charAt(agg_len + coords.x) === "\n") {
    return agg_len + coords.x + 1;
  } else {
    return agg_len + coords.x;
  }
};

// Utility function for converting coordinates (coords) into offsets.
Textarea.prototype._convertToOffset = function(lines, coords) {
  var offset = { x: null, y: null };
  offset.x = this.strWidth(lines[coords.y].slice(0, coords.x));
  offset.y = coords.y - (this.childBase || 0);
  return offset;
};

// Utility function to calculate the width of the previous glyph.
Textarea.prototype._getPrevGlyphLength = function(is_unicode, text, pos) {
  ...
  if (pos == 0) {
    ...
    return this._getGlyphLength(is_unicode, text.slice(pos - 2, pos));
  }
};

// Utility function to calculate the width of the next glyph.
Textarea.prototype._getNextGlyphLength = function(is_unicode, text, pos) {
  if (pos > text.length) throw 'Out of bounds';
  ...
  if (pos == text.length) {
    ...
    return this._getGlyphLength(is_unicode, text.slice(pos, pos + 2));
  }
};

// Utility function to calculate the width of a potential two-character glyph
// (two_chars).
Textarea.prototype._getGlyphLength = function(is_unicode, two_chars) {
  if (!is_unicode) {
    return 1;
...
Textarea.prototype._processKeyPress = function() {
  ...
  case 'up':
    this.cursor = this._getCoordsAfterUp(this._clines, this.cursor);
    this.caret = this._getPositionForCoords(this._clines, this.value, this.cursor);
    break;
  case 'down':
    this.cursor = this._getCoordsAfterDown(this._clines, this.cursor);
    this.caret = this._getPositionForCoords(this._clines, this.value, this.cursor);
    break;
  case 'left':
    this.caret -= this.key_pressed.length;
    this.cursor = this._getCoordsForPosition(this._clines, this.value, this.caret);
    break;
  case 'right':
    this.caret += this.key_pressed.length;
    this.cursor = this._getCoordsForPosition(this._clines, this.value, this.caret);
