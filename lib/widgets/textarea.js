...
Textarea.prototype._getCoordsAfterDown = function(lines, original) {
  if (original.y + 1 == lines.length) return coords(original.x, original.y);
  
  var y = original.y + 1;
  var width = this.strWidth(lines[original.y].slice(0, original.x));
  var x = this._getCoordForWidth(lines[y], width);
  return coords(x, y);
}
...
Textarea.prototype._getCoordsAfterUp = function(lines, original) {
...
Textarea.prototype._convertToOffset = function(lines, original) {
  var x = this.strWidth(lines[original.y].slice(0, original.x));
  var y = original.y - (this.childBase || 0);
  return coords(x, y);
};
