...
Textarea.prototype._updateCursor = function(get) {
  ...
  var offset = this.caret.relative;
...
Textarea.prototype._listener = function(ch, key) {
  ...
  case 'backspace':
    if (!value.length) break;
    var len = this.caret.prevGlyphLength(value, lcar);
    this.value = editStringDelete(value, len, lcar);
    this.pressed.key = 'backspace';
    this.pressed.len = len;
    break;
  default:
    if (ch && !/^[\x00-\x08\x0b-\x0c\x0e-\x1f\x7f]$/.test(ch)) {
      this.value = editStringInsert(value, ch, lcar);
