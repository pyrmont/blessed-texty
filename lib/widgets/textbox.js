...
function Textbox(options) {
  ...
  // The usuable width.
  this.viewable = { _widget: this,
                    get width() { return this._widget.width - this._widget.iwidth - 1; } };
...
Textbox.prototype._updateOffset = function() {
  ...
  if (shift_left) {
    this.offset.x -= this.caret.prevGlyphLength(this.value, this.offset.x);
  } else if (shift_right) {
    this.offset.x += this.caret.nextGlyphLength(this.value, this.offset.x);
  }
};
